...
if __name__ == "__main__":
    ...
    map_ = {}
    explore((0, 0), prog, map_)

    oxygen = list(filter(lambda k: map_[k] == 0, map_))
    assert len(oxygen) == 1
    ...
    while True:
        ...
        for o in oxygen:
            for d in ((0, 1), (0, -1), (-1, 0), (1, 0)):
                n = (o[0] + d[0], o[1] + d[1])
                if n in map_ and map_[n] not in (-1, 0):
                    map_[n] = 0
                    filled.append(n)
        t += 1
        test = list(filter(lambda k: map_[k] != 0 and map_[k] != -1, map_))
        if len(test) == 0:
            break
        oxygen = filled
    print(t)
